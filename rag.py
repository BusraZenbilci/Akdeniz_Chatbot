    ...
    if submit_btn:
        ...
        with col_normal:
            with st.spinner("Yanıt Hazırlanıyor..."):
                ...
                st.markdown(raghelper.ask_gpt(prompt=prompt))
                st.divider()
...
st.title("LangChain ile Bellek Genişletme: Excel")
st.divider()

col_input, col_rag, col_normal = st.columns([1,2,2])

with col_input:
    selected_file = st.file_uploader(label="İşlenecek Dosyayı Seçiniz", type=["xlsx", "xls"])
    st.divider()
    prompt = st.text_input(label="Sorunuzu Giriniz:", key="excel_prompt")
    st.divider()
    submit_btn = st.button(label="Sor", key="excel_button")
    st.divider()
